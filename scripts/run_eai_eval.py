...
def main():
    ...
    parser.add_argument("--agent_id", type=int, help="ID of the agent to use for generating trajectories.")
                ...
                for i in range(args.num_candidates):
                    ...
                    if args.agent_id is not None:
                        candidate_output = generator.generate_trajectory_from_prompt(
                            ...
                            agent_id=args.agent_id  # 使用指定的agent_id
                        )
                    else:
                        # 如果没有指定agent_id，使用默认的agent 0
                    candidate_output = generator.generate_trajectory_from_prompt(
                        prompt_text=instruction,
                            task_type=args.task_type,
                            agent_id=0
