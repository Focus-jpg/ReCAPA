...
class ReflectionHead(nn.Module):
    ...
    def forward(
        ...
        subgoal_score_weight: float, # 新增：中层分数场权重
        ...
        :param subgoal_score_weight: a float for mid-level subgoal score field loss weight
        ...
        loss_breakdown["contrastive_loss"] = contrastive_loss.item()
        ...
        if level == 'high':
            ...
            total_loss += sinkhorn_weight * sinkhorn_loss
            loss_breakdown["sinkhorn_loss"] = sinkhorn_loss.item()
        ...
        if level == 'low':
            ...
            total_loss += score_field_weight * score_loss
            loss_breakdown["score_field_loss"] = score_loss.item()

        if level == 'mid':
            # 中层应用新的子目标分数场损失
            # 需要从pos_trajs和neg_trajs中获取嵌入
            # 假设pos_trajs[0]是正样本, neg_trajs是负样本列表
            low_level_traj_emb = self.rce_module.encode_trajectory(anchor_traj, level='low') # 用低层编码器获取表示
            positive_mid_level_emb = self.rce_module.encode_trajectory(pos_trajs[0], level='mid')
            
            negative_mid_level_embs_list = [self.rce_module.encode_trajectory(t, level='mid') for t in neg_trajs]
            negative_mid_level_embs = torch.stack(negative_mid_level_embs_list, dim=1)

            subgoal_score_loss = self.rce_module.compute_subgoal_score_loss(
                low_level_traj_emb=low_level_traj_emb,
                positive_mid_level_emb=positive_mid_level_emb,
                negative_mid_level_embs=negative_mid_level_embs
            )
            total_loss += subgoal_score_weight * subgoal_score_loss
            loss_breakdown["subgoal_score_loss"] = subgoal_score_loss.item()


        # 3. CGF: Map gradients of total loss to the policy network
        self.cgf_module(
            loss=total_loss,
